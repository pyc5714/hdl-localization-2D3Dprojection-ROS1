...
public:
  ...
  void imageCallback(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    int image_width = msg->width;
      ...
      // Create a 2D vector to store the pixel class values
    // pixel_classes.resize(image_height, std::vector<int>(image_width));
    // if (msg->header.stamp.secs == )

    // Extract the pixel class values from the class_msg and store them in pixel_classes
    for (int row = 0; row < image_height; ++row)
    {
        for (int col = 0; col < image_width; ++col)
        {
            pixel_classes[row][col] = static_cast<int>(msg->data[row * image_width + col]);
            // std::cout << pixel_classes[row][col] << " , ";
        }
        std:cout << endl ;
    }
    // cout << pixel_classes[10][10] << endl; 


    // Continue processing with the undistorted image
  }
  ...
  void pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
  {
    ...
    *laser_cloud_in = *laser_cloud_in_filter;   // laser_cloud_in_filter를 다시 laser_cloud_in에 넣음
    ...
    // 6. save new cloud
    double timeScanCur = cloud_msg->header.stamp.toSec();
    cloudQueue.push_back(*laser_cloud_in);
    timeQueue.push_back(timeScanCur);
    // Remove outdated point clouds from the queue
    while (!timeQueue.empty() && timeScanCur - timeQueue.front() > 0.1)
    {
    ...
    *depthCloud = *depthCloudDS;
    ...
    // Eigen::Affine3f transOffset = pcl::getTransformation(L_C_TX, L_C_TY, L_C_TZ, L_C_RX, L_C_RY, L_C_RZ);
    pcl::transformPointCloud(*depthCloud, *transformed_cloud, transOffset);
    ...
    publishRGBCloud(&pub_1, outside_image_RGB_cloud, cloud_msg->header.stamp, "velodyne");
    outside_image_RGB_cloud->clear();
  }
...
int main(int argc, char **argv)
{
  ...
  //   // // cv::Mat img = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::MONO8)->image;
  //   // // cv::Mat img = cv_bridge::toCvCopy(msg, "mono8")->image;

  //   // // Correct distortion
  //   // cv::Mat undistorted_img;
  //   // cv::undistort(img, undistorted_img, camera_matrix, dist_coeffs);
    
  //   int image_height = msg->height;
  //   int image_width = msg->width;
  //     // std::cout << "height is  : " << image_height << endl;
  //     // std::cout << "image_width is  : " << image_width << endl;

  //     // Create a 2D vector to store the pixel class values
  //   pixel_classes.resize(image_height, std::vector<uint8_t>(image_width));
  ...
  //       for (int col = 0; col < image_width; ++col)
  //       {
  //           pixel_classes[row][col] = msg->data[row * image_width + col];
  //           std::cout << static_cast<int>(msg->data[row * image_width + col]) << " , ";
  //       }
  //       std:cout << endl ;
  //   }
  //   // cout << pixel_classes[10][10] << endl; 
